...
Public DBSeparate As String

Public CharacterCollation As String
...
Function DataBase_Init()
  ...
  Framework = Worksheets("設定").Range("B16").Value
  SaveDirPath = Worksheets("設定").Range("B12")
  CharacterSet = Worksheets("設定").Range("B13").Value
  CharacterCollation = Worksheets("設定").Range("B14").Value
  Framework = Worksheets("設定").Range("B15").Value
  
  Select Case CharacterCollation
  Case "CRLF"
    LineSeparator = -1
...
Function DataBase_MakeSQL(SetDisplyAlertFlg As Boolean)
  ...
  Select Case DBMS
    ...
    Case "MySQL"
      ...
      ObjADODB.WriteText LineBreakCode & "DROP TABLE IF EXISTS " & TableID + ";" & LineBreakCode, 1
...
'      ObjADODB.WriteText "END;", 1
      ...
      ObjADODB.WriteText LineBreakCode & "-- DROP TABLE " & TableID + " CASCADE CONSTRAINTS PURGE;", 1
  ...
  'カラムの文字数確認
  DB_ColumnNameLen = Len(Range("E9"))
  DB_ColumnTypeLen = Len(Range("G9"))
  
  
  Do While (Range("E" & rowLine).Value <> "")
    If Range("G" & rowLine) <> "" Then
      If DB_ColumnNameLen <= Len(Range("E" & rowLine)) Then
        DB_ColumnNameLen = Len(Range("E" & rowLine))
      End If
  ...
  Do While (Range("E" & rowLine).Value <> "")
    If Range("G" & rowLine) <> "" Then
      'カラム名取得
      DB_ColumnName = Library_GetFixlng(DBSeparate & Range("E" & rowLine) & DBSeparate & Space, DB_ColumnNameLen + 4)
'      DB_ColumnName = Range("E" & rowLine)
      ...
      Select Case DB_ColumnType
        ...
        Case "auto_increment"
          WriteBuffTmp = "int auto_increment"
        Case Else
      ...
      End If
      WriteBuff = "  " & DB_ColumnName & "  " & WriteBuffTmp
  ...
  If objPrimaryKey.count > 0 Then
    ...
    Next i
      ...
      Select Case DBMS
        ...
        Case "MySQL"
          WriteBuff = LineBreakCode & "  ,PRIMARY KEY (" & PrimaryKeyNames & ")" & LineBreakCode & ")"
          ObjADODB.WriteText WriteBuff, 1
  ...
  Select Case DBMS
    ...
    Case "MySQL"
'      ObjADODB.WriteText "ENGINE=InnoDB DEFAULT CHARSET=" & Replace(CharacterSet, "-", "") & " COLLATE='" & CharacterCollation & "' COMMENT ='" & TableName & "';", 1
      ObjADODB.WriteText "ENGINE=InnoDB COMMENT ='" & TableName & "';", 1
  ...
  If SetDisplyAlertFlg = True Then
    ans = MsgBox(Range("D5") + "用スクリプトファイルの作成が完了しました。" & LineBreakCode & SaveDirPath & "を開きますか？", vbYesNo)
...
'      PostgreSQL_MakeDDL
    ...
    Case "MySQL"
      ...
      SelectString = SelectString & "    , EXTRA                                AS EXTRA    " & LineBreakCode
  ...
  Do Until DBRecordset.EOF
    ...
    Extra = DBRecordset.Fields("EXTRA").Value
    Select Case Extra
      Case "auto_increment"
...
'      ColumnDefault = Replace(ColumnDefault, "'", "")
      ...
      Select Case Extra
        Case "on update CURRENT_TIMESTAMP"
          dataType = "CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP"
      End Select
    ...
    Else
      Select Case DBMS
        ...
        Case "MySQL"
          ...
          IndexType = DBRecordset.Fields("index_type").Value
      ...
      Range("F" & NowLine).Value = IndexType
...
'  Loop
  ...
  ProgressBar_ProgShowCount TableName & " 処理中・・・", 1, 5, " DB切断中"
  ...
  '不要行の削除
  ProgressBar_ProgShowCount TableName & " 処理中・・・", 2, 5, "行数調整"
'  DataBase_SetSheetStyle
  
  ProgressBar_ProgShowCount TableName & " 処理中・・・", 3, 5, "印刷範囲設定"
...
Sub DataBase_MakeAllSheetFlameworkSQL()
  ...
  ans = MsgBox("スクリプトファイルの作成が完了しました。" & LineBreakCode & SaveDirPath & "を開きますか？", vbYesNo)
  If ans = vbYes Then
      Call Shell("Explorer.exe  " & SaveDirPath, vbNormalFocus)
  End If
End Sub
...
' * @author Bunpei.Koizumi<bunpei.koizumi@gmail.com>
'***************************************************************************************************************************************************
Function DataBase_MakeFlameworkSQL(SetDisplyAlertFlg As Boolean)
...
'  Dim objIndex4 As New Dictionary
  ...
  On Error GoTo MakeFlameworkSQLError:
  ...
  ObjADODB.WriteText "<?php", 1
  ObjADODB.WriteText "use Illuminate\Database\Schema\Blueprint;", 1
  ObjADODB.WriteText "use Illuminate\Database\Migrations\Migration;", 1
  ObjADODB.WriteText "class Create" & Library_FirstCharConvert(Library_SnakeToCamel(TableID)) & "Table extends Migration {", 1
  ObjADODB.WriteText "", 1
  ObjADODB.WriteText "    /**", 1
  ObjADODB.WriteText "     * Run the migrations.", 1
  ObjADODB.WriteText "     *", 1
  ObjADODB.WriteText "     * @return void", 1
  ObjADODB.WriteText "     */", 1
  ObjADODB.WriteText "    public function up() {", 1
  ObjADODB.WriteText "        Schema::create ( '" & TableID & "',", 1
  ObjADODB.WriteText "                function (Blueprint $table)  {", 1
...
'      DB_ColumnName = Library_GetFixlng(DBSeparate & Range("E" & rowLine) & DBSeparate & Space, DB_ColumnNameLen + 4)
      ...
      Select Case DB_ColumnType
        Case "auto_increment"
          DB_ColumnType = "increments"
        Case "varchar"
          DB_ColumnType = "string"
        Case "int"
          DB_ColumnType = "integer"
      End Select
      WriteBuff = "                    $table->" & DB_ColumnType & "('" & DB_ColumnName & "'"

      '桁数の設定有無を判定
      If DB_ColumnDigit <> "" Then
        WriteBuff = WriteBuff & ", " & DB_ColumnDigit & ")"
      Else
        WriteBuff = WriteBuff & ")"
      End If
      ...
      'NOT NULLの設定有無を判定---------------------------------------------------------------------------------------------------------------
      If DB_NotNull <> 1 Then
        WriteBuff = WriteBuff & "->nullable()"
      End If
    
      ' 初期値設定----------------------------------------------------------------------------------------------------------------------------
      If DB_ColumnDefValue <> "" Then
        Select Case DB_ColumnDefValue
          Case "CURRENT_TIMESTAMP"
            WriteBuff = WriteBuff & "->default(\DB::raw('CURRENT_TIMESTAMP'))"
          Case "CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP"
            WriteBuff = WriteBuff & "->default(\DB::raw('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'))"
          Case Else
            If IsNumeric(DB_ColumnDefValue) Then
              WriteBuff = WriteBuff & "->default(" & DB_ColumnDefValue & ")"
            Else
              WriteBuff = WriteBuff & "->default('" & DB_ColumnDefValue & "')"
            End If
...
Loop
  ...
  ObjADODB.WriteText "                } ); ", 1
  ObjADODB.WriteText "                DB::statement(""ALTER TABLE " & TableID & " COMMENT '" & TableName & "'"");", 1
  
  
  ObjADODB.WriteText "    } ", 1
  ObjADODB.WriteText " ", 1
  ObjADODB.WriteText "    /** ", 1
  ObjADODB.WriteText "     * Reverse the migrations. ", 1
  ObjADODB.WriteText "     * ", 1
  ObjADODB.WriteText "     * @return void ", 1
  ObjADODB.WriteText "     */ ", 1
  ObjADODB.WriteText "    public function down() { ", 1
  ObjADODB.WriteText "        Schema::dropIfExists ( '" & TableID & "' ); ", 1
  ObjADODB.WriteText "    } ", 1
  ObjADODB.WriteText "} ", 1
  ...
  If Range("X6") <> "" Then
    fileName = Range("X6")
  Else
    fileName = Format(Now, "yyyy_mm_dd_hhnnss")
    Range("X6") = fileName
  End If
  fileName = fileName & "_Create" & Library_FirstCharConvert(Library_SnakeToCamel(TableID)) & "Table.php"
  
  ObjADODB.SaveToFile (SaveDirPath & "\" & fileName), 2
  ...
  If SetDisplyAlertFlg = True Then
    ...
    If ans = vbYes Then
        Call Shell("Explorer.exe /select, " & SaveDirPath & "\" & fileName, vbNormalFocus)
    End If
  End If
  
  Exit Function
  
MakeFlameworkSQLError:
