...
' * @author Bunpei.Koizumi<bunpei.koizumi@gmail.com>
'**************************************************************************************************
Function errorHandle(id As Long, Message As String)
...
' * @author Bunpei.Koizumi<bunpei.koizumi@gmail.com>
'**************************************************************************************************
Function endScript(Optional flg As Boolean)
...
'  アクティブセルの選択
  If SelectionCell <> "" And IsMissing(flg) Then
...
'**************************************************************************************************
Function chkBookOpened(chkFile As String) As Boolean
...
'**************************************************************************************************
Function chkFileExists(chkFile As String) As Boolean
  Dim FSO As Object
...
' * ディレクトリが存在するか
' *
' * @Link http://officetanaka.net/excel/vba/filesystemobject/filesystemobject11.htm
'**************************************************************************************************
Function chkDirExists(chkDir As String) As Boolean
  Dim FSO As Object
...
' * @author Bunpei.Koizumi<bunpei.koizumi@gmail.com>
'**************************************************************************************************
Function closeExpansionForm(Text As String, SetSelectTargetRows As String)
...
'**************************************************************************************************
Function getColumnName(ByVal pColumn As Long) As String

  Dim oTarget  As Range
  Dim sAddress As String
  Dim lLength  As Long

  If pColumn < 1 Or pColumn > Worksheets(1).Cells.Columns.count Then
    getColumnName = ""
    Exit Function
  End If
  Set oTarget = Worksheets(1).Columns(pColumn)
  sAddress = oTarget.Address(False, False, xlA1)
  lLength = InStr(1, sAddress, ":") - 1
  getColumnName = Left(sAddress, lLength)
End Function
...
' * @author Bunpei.Koizumi<bunpei.koizumi@gmail.com>
'**************************************************************************************************
Function getRGB(colorValue, Red, Green, Blue)
...
Function getRegistry(registryName As String)
  ...
  regVal = GetSetting(RegistryKey, RegistrySubKey, registryName, 0)
  getRegistry = regVal
End Function
...
' * @author Bunpei.Koizumi<bunpei.koizumi@gmail.com>
'**************************************************************************************************
Function getDirPath(CurrentDirectory As String, title As String)
...
' * @author Bunpei.Koizumi<bunpei.koizumi@gmail.com>
'**************************************************************************************************
Function getFilePath(CurrentDirectory As String, SaveFileName As String, FileTypeNo As Long)
...
GetSheetListError:
  ...
  ' 画面描写制御終了
  Call endScript
  Call errorHandle(Err.Number, Err.Description)
...
' * @author Bunpei.Koizumi<bunpei.koizumi@gmail.com>
'**************************************************************************************************
Function makeRandomNumber(ByVal setString As String, ByVal setStringCnt As Integer) As String
...
Function outputLog(Message As String)
  ...
  logFile = logPath & "\excelMethod_" & Format(Now, "YYYY-MM-DD") & ".log"

  'ログファイルが存在しなければ、ログローテート確認
  If Dir(logFile) = "" Then
    tmp = Dir(logPath & "\excelMethod_*.log")
    Do While tmp <> ""
      oldLogFile = Replace(tmp, "excelMethod_", "")
      oldLogFile = Replace(oldLogFile, ".log", "")
      oldLogFile = Replace(oldLogFile, "-", "/")

      If DateDiff("d", oldLogFile, Now) > logrotate Then
        Kill Path & "\" & tmp
      End If
      tmp = Dir()
    Loop
...
' * @author Bunpei.Koizumi<bunpei.koizumi@gmail.com>
'**************************************************************************************************
Function importCsv(filePath As String)
...
' * @author Bunpei.Koizumi<bunpei.koizumi@gmail.com>
'**************************************************************************************************
Function importXlsx(filePath As String, targeSheet As String, targeArea As String, dictSheet As Worksheet, passWord As String)
...
'**************************************************************************************************
Function resetComment(pBarTitle As String)
    Dim cl As Range
    Dim count As Long
...
' * クリップボードクリア
' *
' * @Link   http://www.excel.studio-kazu.jp/kw/20110914223621.html
'***********************************************************************************************************************************************
...
'**************************************************************************************************
Public Function Lib_TEXTJOIN(Delim, Ignore As Boolean, ParamArray par())
  Dim i As Integer
  Dim tR As Range
